...
PluginGUI::PluginGUI(PlayerList &_playerList)
    : mainWindow(false), moveLeftAtScroll(false), playerList(_playerList) {}
...
void PluginGUI::drawHeader() {
  ...
  drawList->AddText(ImVec2(pos.x + 4.f, pos.y - 20.f), -1,
                    samp::RefNetGame()->m_szHostname);
  drawList->AddText(ImVec2(pos.x + width - toSize.x - 4.f, pos.y - 20.f), -1,
                    textOnline.c_str());
  ...
  drawList->AddText(
      ImVec2(pos.x + 22.f - id.x / 2.f, pos.y + height / 2.f - 8.f), -1, "ID");
  drawList->AddText(ImVec2(pos.x + 44.f, pos.y + height / 2.f - 8.f), -1,
                    "NICKNAME");
  drawList->AddText(
      ImVec2(
          pos.x + width - 150.f - score.x / 2.f -
              (moveLeftAtScroll ? ImGui::GetStyle().ScrollbarSize + 2.f : 0.f),
          pos.y + height / 2.f - 8.f),
      -1, "SCORE");
  drawList->AddText(
      ImVec2(
          pos.x + width - 60.f - ping.x / 2.f -
              (moveLeftAtScroll ? ImGui::GetStyle().ScrollbarSize + 2.f : 0.f),
          pos.y + height / 2.f - 8.f),
      -1, "PING");
  ImGui::Dummy(ImVec2(width, height + 2.f));
}

void PluginGUI::drawBox(int id, std::string nick, int score, int ping,
                        ImU32 color) {
  auto windowSize = ImGui::GetIO().DisplaySize;
  auto drawList = ImGui::GetWindowDrawList();
  auto pos = ImGui::GetCursorScreenPos();
  ...
  bool hovered =
      ImGui::IsMouseHoveringRect(pos, ImVec2(pos.x + width, pos.y + height));

  drawList->AddRectFilled(pos, ImVec2(pos.x + width, pos.y + height),
                          0xcc010101);
  drawList->AddRectFilled(
      pos, ImVec2(pos.x + 5.5, pos.y + height),
      ImGui::ColorConvertFloat4ToU32(ImVec4(col.x, col.y, col.z, 0.8f)));

  if (hovered) {
    drawList->AddRectFilledMultiColor(
        ImVec2(pos.x + 5.5, pos.y), ImVec2(pos.x + width, pos.y + height),
    ...
    if (ImGui::IsMouseDoubleClicked(0)) {
      ...
      rpc.Write(id);
      rpc.Write(0);
      rakhook::send_rpc(22, &rpc, PacketPriority::HIGH_PRIORITY,
                        PacketReliability::UNRELIABLE_SEQUENCED, (char)0,
                        false);
  ...
  drawList->AddText(
      ImVec2(pos.x + 22.f - idSize.x / 2.f, pos.y + height / 2.f - 8.f), color,
      idStr.c_str());
  drawList->AddText(ImVec2(pos.x + 44.f, pos.y + height / 2.f - 8.f), color,
                    nick.c_str());
  drawList->AddText(ImVec2(pos.x + width - 150.f - scoreSize.x / 2.f,
                           pos.y + height / 2.f - 8.f),
                    color, scoreStr.c_str());
  drawList->AddText(ImVec2(pos.x + width - 60.f - pingSize.x / 2.f,
                           pos.y + height / 2.f - 8.f),
                    color, pingStr.c_str());
  ImGui::Dummy(ImVec2(width, height + 2.f));
}
...
void PluginGUI::process() {
  ...
  drawHeader();
  ImGui::SetCursorPosX(windowSize.x / 2.f - width / 2.f);
  ImGui::BeginChild("##scrollArea",
                    ImVec2(width, windowSize.y - 120.f - height));
  auto players = samp::RefNetGame()->GetPlayerPool();
  drawBox(players->m_localInfo.m_nId, players->m_localInfo.m_szName,
          players->GetLocalPlayerScore(), players->GetLocalPlayerPing(),
  ...
  while (clipper.Step()) {
    ...
    for (int i = clipper.DisplayStart; i < clipper.DisplayEnd; ++i, ++it) {
      auto id = *it;
      auto players = samp::RefNetGame()->GetPlayerPool();
      drawBox(id, players->GetName(id), players->GetScore(id),
              players->GetPing(id),
