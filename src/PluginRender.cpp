...
HWND gameHwnd = []() {
  ...
  if (pHwnd != nullptr) {
    ...
    hookGameInstanceInit.after +=
        [](const auto &hook, HWND &returnValue, HINSTANCE inst) {
...
PluginRender::PluginRender(PlayerList &_playerList)
    : ImGuiinited(false), playerList(_playerList), GUI(playerList) {
  using namespace std::placeholders;
  hookPresent.set_dest(getFunctionAddress(17));
  hookReset.set_dest(getFunctionAddress(16));
  hookPresent.before +=
      std::bind(&PluginRender::onPresent, this, _1, _2, _3, _4, _5, _6);
  ...
  static std::uintptr_t base = [](std::size_t len) {
    ...
    if (auto size = GetSystemDirectoryA(pathTo.data(), MAX_PATH)) {
      ...
      std::uintptr_t dwObjBase =
          reinterpret_cast<std::uintptr_t>(LoadLibraryA(pathTo.c_str()));
...
PluginRender::onPresent(const decltype(hookPresent) &hook,
                        IDirect3DDevice9 *pDevice, const RECT *, const RECT *,
                        HWND, const RGNDATA *) {
  if (!ImGuiinited && samp::RefNetGame() != nullptr) {
    ...
    hookWndproc.set_cb(
        std::bind(&PluginRender::onWndproc, this, _1, _2, _3, _4, _5));
    hookWndproc.install();
    ...
    io.Fonts->AddFontFromFileTTF("C:\\Windows\\Fonts\\trebucbd.ttf", 16.0f,
                                 &font_config,
...
std::optional<HRESULT> PluginRender::onLost(const decltype(hookReset) &hook,
...
void PluginRender::onReset(const decltype(hookReset) &hook,
                           HRESULT &returnValue, IDirect3DDevice9 *pDevice,
                           D3DPRESENT_PARAMETERS *parameters) {}

HRESULT __stdcall PluginRender::onWndproc(const decltype(hookWndproc) &hook,
  ...
  case WM_KEYDOWN: {
    if (wParam == VK_TAB && (HIWORD(lParam) & KF_REPEAT) != KF_REPEAT) {
      ...
      samp::RefGame()->SetCursorMode(GUI.mainWindow ? samp::CURSOR_LOCKCAM
                                                    : samp::CURSOR_NONE,
                                     !GUI.mainWindow);
      samp::RefChat()->m_nMode = GUI.mainWindow
                                     ? samp::RefChat()->DISPLAY_MODE_OFF
                                     : samp::RefChat()->DISPLAY_MODE_NORMAL;
      return 1;
